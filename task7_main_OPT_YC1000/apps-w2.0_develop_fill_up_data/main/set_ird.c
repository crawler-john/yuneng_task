...
int set_ird_single(char *inverter_id, int shortaddr)		//设置单台逆变器IRD
{
	char value[16]={'\0'};

	shortaddr = get_ird_id_value(inverter_id, value);

	if (0!=shortaddr)
	{
		clear_ird_flag_single(inverter_id);
		send_ird_command_single(shortaddr, atoi(value));	//设置一台逆变器IRD
		get_ird_single(shortaddr,inverter_id);			//读取逆变器的设置结果
		return 1;
	}
	else {
		clear_ird_flag_single(inverter_id);
