...
/*发送包给所有逆变器*/
int send_package_to_single(char *id, int speed)
{
	int i, fd, package_num=0;
	char sendbuff[256]={'\0'};
	unsigned char package_buff[128];
...
/*补发丢失的包*/
int resend_lost_packets(char *id, int speed)
{
				...
				else if((readbuff[23]*256 + readbuff[24]) > 0)
				{
					...
					if(fd>0)
					{
						while((readbuff[23]*256 + readbuff[24]) > 0)
						{
							...
							{
								write(plcmodem, sendbuff, 28+speed);
								printhexmsg("sendbuff", sendbuff, 60);
...
int update_inverter()
{
	...
	if(SQLITE_OK == sqlite3_get_table( db , sql , &azResult , &nrow , &ncolumn , &zErrMsg ))
	{
		for(i=1; i<=nrow; i++)
		{
			...
			{
				send_package_to_single(azResult[i], speed);
				resend_lost_packets(azResult[i], speed);
