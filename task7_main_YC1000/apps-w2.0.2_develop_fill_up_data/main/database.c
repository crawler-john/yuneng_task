...
sqlite3 *init_database()			//数据库初始化
{
	...
	strcpy(sql,"CREATE TABLE IF NOT EXISTS id"
			...
			"model INTEGER, software_version INTEGER, sub_version INTEGER,"
...
int get_id_from_db(inverter_info *firstinverter)	//获取逆变器的ID号
{
	...
	strcpy(sql,"SELECT id,short_address,model,software_version,bind_zigbee_flag,turned_off_rpt_flag FROM id");
	sqlite3_get_table( db , sql , &azResult , &nrow , &ncolumn , &zErrMsg );
	...
	for(i=1;i<=nrow;i++)
	{
		...
		{
			inverter->version = atoi(azResult[i*ncolumn+3]);
		}
		...
		{
			inverter->bindflag = atoi(azResult[i*ncolumn+4]);
		}

		if(NULL==azResult[i*ncolumn+5])
		{
			inverter->zigbee_version = 0;		//没有获取到ZIGBEE版本号的逆变器赋值为0.ZK
		}
		else
		{
			inverter->zigbee_version = atoi(azResult[i*ncolumn+5]);
		}
		inverter++;
	}
